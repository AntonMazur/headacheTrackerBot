...
# async def save_medication(callback: CallbackQuery):
#     user_id = callback.from_user.id
#     user_data[user_id]['medication'] = "Yes" if callback.data == "medication_yes" else "No"
...
#         # cursor.execute(
#         #     "INSERT INTO headaches (date, start_time, stop_time, medication, rating, comments) VALUES (?, ?, ?, ?, ?, ?)",
#         #     (data['date'], data['start_time'], data['stop_time'], data['medication'], data['rating'], data.get('comments', '')))
#         # conn.commit()
#         # await message.answer("Your headache record has been saved.")
#         if 'medications' in data:
#             medications_report = "\n".join([
#                 f"{med['name']} - Taken at: {med['time']}" for med in data['medications']
#             ])
#         else:
#             medications_report = "No medications recorded."
#         # Insert data into the database
#         cursor.execute(
#                 "INSERT INTO headaches (date, start_time, stop_time, medications, rating, comments) VALUES (?, ?, ?, ?, ?, ?)",
#                 (data['date'], data['start_time'], data['stop_time'], medications_report, data['rating'],
#                  data.get('comments', '')))
#         conn.commit()
#         await message.answer("Your headache record has been saved.")
#     else:
...
#             try:
#                 datetime.strptime(message.text, "%H:%M")  # Validate time format
...
#
#         elif 'stop_time' not in user_data[user_id]:
#             try:
#                 datetime.strptime(message.text, "%H:%M")  # Validate time format
...
#                 await message.answer("Invalid format. Please enter time as HH:MM (e.g., 14:30).")
#
#         elif 'comments' not in user_data[user_id]:
...
#     await export_pdf(callback.from_user.id)
#     await callback.answer()
#
# async def export_pdf(chat_id: int):
#     cursor.execute("SELECT date, start_time, stop_time, medication, rating, comments FROM headaches")
#     records = cursor.fetchall()
#
#     if not records:
#         await bot.send_message(chat_id, "No records to export.")
#         return
#
#     pdf = FPDF()
#     pdf.add_page()
#     pdf.set_font("Arial", size=12)
#     pdf.cell(200, 10, "Headache Report", ln=True, align='C')
#
#     # Add header row
#     headers = ["#", "Date", "Start Time", "Stop Time", "Medication", "Rating", "Comments"]
#     pdf.ln(10)
#
...
#         date, start_time, stop_time, medication, rating, comments = record
#
#         # Add each row horizontally
#         pdf.cell(28, 10, str(idx), border=1, align='C')  # Row number
#         pdf.cell(28, 10, date, border=1, align='C')
#         pdf.cell(28, 10, start_time, border=1, align='C')
#         pdf.cell(28, 10, stop_time, border=1, align='C')
#         pdf.cell(28, 10, medication, border=1, align='C')
#         pdf.cell(28, 10, str(rating), border=1, align='C')
#         pdf.cell(28, 10, comments, border=1, align='C')
#         pdf.ln()
#
